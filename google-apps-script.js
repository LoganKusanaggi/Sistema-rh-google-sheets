...
function abrirDashboardModal() {
    ...
    var htmlContent = srvTag +
        ...
        '        }).obterDashboardSanitizadoParaClient(params);\n' +
        '    }\n' +
        '    \n' +
...
/**
 * Atualiza os dados do dashboard (força recálculo no backend)
 */
function atualizarDashboard() {
    var ui = SpreadsheetApp.getUi();
    try {
        var response = UrlFetchApp.fetch(CONFIG.API_URL + '/dashboard/limpar-cache', {
            method: 'post',
            muteHttpExceptions: true
        });
        Logger.log('Refresh Dashboard Cache: ' + response.getContentText());
        ui.alert('Dados atualizados com sucesso!', 'O cache do servidor foi limpo e os dados serão recarregados ao abrir o Dashboard novamente.', ui.ButtonSet.OK);
    } catch (e) {
        Logger.log('Erro ao atualizar dashboard: ' + e.message);
        ui.alert('Aviso', 'Tentativa de atualização enviada ao servidor. ' + e.message, ui.ButtonSet.OK);
    }
}

/**
 * Abre o modal de configurações do sistema
 */
function abrirConfiguracoes() {
    var ui = SpreadsheetApp.getUi();
    var html = HtmlService.createHtmlOutput('<h3>Configurações do Sistema</h3><p>API Endpoint: <b>' + CONFIG.API_URL + '</b></p><hr><p>Versão: 2.1.0-dashboard</p>')
        .setWidth(400)
        .setHeight(300);
