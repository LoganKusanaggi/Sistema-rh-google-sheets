function criarPlanilhaLancamentoFolha(cpfs, periodo, dadosMap = null) {
    ...
    // Mapear índices da aba Colaboradores (baseado na função novoColaboradorModal e renderização)
    // A=Check, B=CPF, C=Nome, D=Email, E=Tel, F=Salário, G=Cargo, H=Depto
    // Então Salário é índice 5 (Coluna F)
    ...
    dadosPlanilha.forEach((row, i) => {
        ...
        if (!mapDados[cpfLimpo].salario_base) mapDados[cpfLimpo].salario_base = row[5]; // Coluna F
