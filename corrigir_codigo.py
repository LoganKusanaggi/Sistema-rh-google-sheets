...
# Encontrar onde começa a função criarPlanilhaLancamentoFolha
inicio_criar = codigo.find('function criarPlanilhaLancamentoFolha(cpfs, periodo) {')
if inicio_criar == -1:
    print("❌ Função criarPlanilhaLancamentoFolha não encontrada!")
    exit(1)

# Encontrar o final da função (procurar pelo próximo 'function' após ela)
temp = codigo[inicio_criar:]
# Encontrar a próxima função
proxima_func = re.search(r'\n\nfunction ', temp[100:])  # Pular os primeiros 100 chars
if proxima_func:
    fim_criar = inicio_criar + 100 + proxima_func.start()
else:
    print("❌ Não encontrei o fim da função criarPlanilhaLancamentoFolha")
    exit(1)

print(f"✅ Função criarPlanilhaLancamentoFolha encontrada: linhas {inicio_criar} a {fim_criar}")

# Encontrar função enviarFolhaParaAPI
inicio_enviar = codigo.find('function enviarFolhaParaAPI() {')
if inicio_enviar == -1:
    print("❌ Função enviarFolhaParaAPI não encontrada!")
    exit(1)

temp2 = codigo[inicio_enviar:]
proxima_func2 = re.search(r'\n\nfunction ', temp2[100:])
if proxima_func2:
    fim_enviar = inicio_enviar + 100 + proxima_func2.start()
else:
    print("❌ Não encontrei o fim da função enviarFolhaParaAPI")
    exit(1)

print(f"✅ Função enviarFolhaParaAPI encontrada: linhas {inicio_enviar} a {fim_enviar}")

# Nova função criarPlanilhaLancamentoFolha
nova_criar = '''function criarPlanilhaLancamentoFolha(cpfs, periodo) {
...
nova_enviar = '''function enviarFolhaParaAPI() {
...
# Substituir
parte1 = codigo[:inicio_criar]
parte2 = codigo[fim_criar:inicio_enviar]
parte3 = codigo[fim_enviar:]

codigo_final = parte1 + nova_criar + parte2 + nova_enviar + parte3

# Salvar
with open('google-apps-script-ATUALIZADO-CORRIGIDO.js', 'w', encoding='utf-8') as f:
    f.write(codigo_final)

print(f"✅ Arquivo criado: google-apps-script-ATUALIZADO-CORRIGIDO.js")
print(f"Tamanho: {len(codigo_final)} caracteres")
