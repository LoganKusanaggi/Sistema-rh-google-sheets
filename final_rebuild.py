...
with open('Versão funcional do AppScript RH.txt', 'r', encoding='utf-8') as f:
    base = f.read()

new_buscar_api = """function buscarDashboardAPI(filtros) {
    try {
        var options = {
            'method': 'get',
...
new_obter_sanitizado = """function obterDashboardSanitizadoParaClient(filtros) {
    try {
...
filter_html = """  <div id="filtros_bar" style="display:flex;gap:12px;align-items:center;padding:10px 24px;background:var(--bg-card);border-bottom:1px solid var(--border);flex-wrap:wrap;">
    ...
    <div style="display:flex;align-items:center;gap:8px;">
        ...
        <select id="f_depto" style="padding:6px 10px;border:1px solid var(--border);border-radius:6px;font-size:13px;">
            ...
            <option value="Comercial">Comercial</option><option value="RH">RH</option>
            <option value="Financeiro">Financeiro</option><option value="Operações">Operações</option>
        </select>
    </div>
...
js_funcs = """    function aplicarFiltros() {
...
result = base
result = re.sub(r'function buscarDashboardAPI\(\) \{[\s\S]+?Logger\.log\(\'Erro Dashboard API: \' \+ e\.message\);[\s\S]+?return \{ success: false, error: e\.message \};\s*\}\s*\}', new_buscar_api, result)

# Use simple string replacement for sections we know
result = result.replace('function abrirDashboardModal() {\n    var ui = SpreadsheetApp.getUi();\n    \n    // PRE-LOAD SERVER SIDE\n    var dashboardData = { success: false, error: "Inicializando" };\n    try {\n        dashboardData = buscarDashboardAPI();\n    } catch(e) {\n        dashboardData = { success: false, error: e.message };\n    }', new_obter_sanitizado)

result = result.replace('  <div class="container" id="content_area">', filter_html + '\n  <div class="container" id="content_area">')
result = result.replace('    function montarInsights(lines) {', js_funcs + '\n\n    function montarInsights(lines) {')

# For the ticker logic, we need to find the if/alertas part
result = re.sub(r'if \(payload\.alertas \&\& payload\.alertas\.length > 0\) \{[\s\S]+?\}', ticker_logic, result)

with open('google-apps-script.js', 'w', encoding='utf-8') as f:
    f.write(result)

print("FINAL REBUILD PY COMPLETE")
